...
const loadFont = async (fontId: string, family: string, isVariable = false, weights?: number[]): Promise<boolean> => {
  try {
...
const FontSelector: React.FC<FontSelectorProps> = ({ value, onChange, onWeightsChange, onFontWeightSupport }) => {
      ...
      try {
        ...
        await Promise.allSettled(
          commonFonts.map(async (fontId) => {
            ...
            if (font) {
              await loadFont(font.id, font.family, font.isVariable, font.weights)
            }
          })
        )
      } catch (error) {
  ...
  const handleSelect = async (fontFamily: string) => {
    ...
    try {
      const success = await loadFont(font.id, font.family, font.isVariable, font.weights)
      if (!success) {
