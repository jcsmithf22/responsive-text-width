...
export type SpacingWithUnit = { top: ValueWithUnit; right: ValueWithUnit; bottom: ValueWithUnit; left: ValueWithUnit }
...
const FitText = (
  ...
  const convertToPixels = useCallback((value: ValueWithUnit): number => {
    ...
    switch (unit) {
      ...
      case "ch":
        const charWidth = canvasRef.current ?
          canvasRef.current.getContext("2d")?.measureText("0").width || fontSize * 0.5 :
          fontSize * 0.5
  ...
  const measureText = useCallback(() => {
    ...
    const lineHeightValue = lineHeight.unit ? convertToPixels(lineHeight) : lineHeight.value * fontSize
  ...
  }, [text, fontSize, fontWeight, letterSpacing, isFontLoaded, convertToPixels, fontFamily, lineHeight, textWidth, viewBoxHeight])
  ...
  const checkFontLoaded = useCallback(async () => {
    try {
      ...
      try {
        ...
        const testString = text + 'AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz0123456789'
        ...
        const isFontAvailable = document.fonts.check(`${fontWeight} ${fontSize}px ${fontFamily}`)
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error loading font'
  ...
  if (textWidth === null || viewBoxHeight === null || !isFontLoaded) return null
      ...
      >
        <div
          style={{
            ...
            lineHeight: lineHeight.unit ? formatValueWithUnit(lineHeight) : lineHeight.value,
